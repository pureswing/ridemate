...
import { RidePost, PostVisibility, RidePostDetailsPackage } from '@/types';
...
export default function EditPackageScreen() {
  ...
  function prefill(post: RidePost) {
    ...
    setQty(details.qty ?? 1);
    if (details.packageSize) setPackageSize(details.packageSize);
    setContentTags(details.contentTags ?? []);
    setHandling(details.handling ?? []);
    setDeclaredValue(details.declaredValue != null ? String(details.declaredValue) : '');
    setPkgConfirmed(details.prohibitedConfirmed ?? []);
    setInspectionOk(details.inspectionOk ?? false);
    setOathAccepted(details.oathAccepted ?? false);
  }
  ...
  // ── Live route map preview — only meaningful once routeChanged, since an
  // unchanged route already shows the real stored map. ──
  ...
  }, [routeChanged, originLat, originLng, destinationLat, destinationLng]);
